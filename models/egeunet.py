...
class group_aggregation_bridge(nn.Module):
    def __init__(self, dim_xh, dim_xl, k_size=3, d_list=[1, 2, 5, 7], mask_mode=False):
        super().__init__()
        self.pre_project = nn.Conv2d(dim_xh, dim_xl, 1)
        group_size = dim_xl // 2 + 1 if mask_mode else dim_xl // 2
        ...
        tail_chn = dim_xl * 2 + 4 if mask_mode else dim_xl * 2
        self.tail_conv = nn.Sequential(
            LayerNorm(normalized_shape=tail_chn, data_format='channels_first'),
            nn.Conv2d(tail_chn, dim_xl, 1)
        )
...
class EGEUNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.final = nn.Conv2d(c_list[0], output_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        if self.gt_ds:
            return (self.final_act(gt_pre5), self.final_act(gt_pre4), self.final_act(gt_pre3), self.final_act(gt_pre2),
                    self.final_act(gt_pre1)), self.final_act(out0)
        else:
            return self.final_act(out0)

if __name__ == '__main__':
    unet = EGEUNet(final_activate='tanh', gt_ds=False)
    unet.eval()
